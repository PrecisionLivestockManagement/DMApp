...
appget_users <- function(email = NULL, fields = NULL, username = NULL, password = NULL){
  ...
  if(is.null(email)){}else{email <- sprintf('"loginemail":"%s",', email)}
...
data <- users$find(query = search, fields = snappy)
#
#  if(length(data) >1){
