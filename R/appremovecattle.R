...
appremovecattle <- function(RFID, MTag, property, paddock, date, username, password){
  ...
  for (i in 1:length(RFID)){
    ...
    padhist <- DMApp::appgetpaddockhistory(cattle_id = banger$`_id`, currentPaddock = "TRUE", username = username, password = password)
    ...
    if(banger$properties$ALMS == "TRUE"){
      almshist <- DMApp::appgetalmshistory(cattle_id = banger$`_id`, currentALMS = "TRUE", username = username, password = password)
