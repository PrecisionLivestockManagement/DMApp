#' Retrieves data from the DataMuster database for the Table of ALMS Data
#'
#' This function retreives the latest weekly weight data from the DataMuster database and prepares the data for table display on the DataMuster website
#' @name appalmstable
...
#' @return a dataframe with a list of cattle RFID numbers, management tags, category and sex information, current paddock allocations and the latest weekly weight and date
...
appalmstable <- function(property, sex, category, paddock, zoom, timezone, username, password){
  ...
  lookfor <- sprintf('{"RFID":true, "properties.Management":true, "properties.Paddock":true, "properties.sex":true, "properties.category":true, "properties.recordedtime":true, "properties.wkweight":true, "properties.wkwtdate":true, "_id":false}')
  ...
  cattleinfof <- cattleinfo%>%
                 ...
                 rename("Tag" = Management, "Sex" = sex, "Category" = category, "Hours since last ALMS record" = recordedtime,
                       "Last Average ALMS Weight Date" = wkwtdate, "Weight (kg)" = wkweight)%>%
