...
appget_weeklywts <- function(RFID = NULL, start = NULL, end = NULL, timezone = NULL, fields = NULL, username = NULL, password = NULL){
...
if(nrow(dataf) != 0){
 ...
 dataf <- dataf%>%
          mutate(avweight = round(as.numeric(avweight), 0),
                 Date = as.Date(Date, tz = "Australia/Brisbane"))%>%
          rename(Weight = avweight)
